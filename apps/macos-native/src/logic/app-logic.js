...
export function normalizeTelemetrySnapshot(rawSnapshot, profiles = []) {
  ...
  return {
    lastUpdatedAt: Number.isFinite(lastUpdatedAt) && lastUpdatedAt > 0 ? Math.trunc(lastUpdatedAt) : null,
    totals,
    byGatewayId,
  };
}
...
export function applyTelemetryEvent(previousSnapshot, gatewayId, eventName) {
  ...
  const currentGatewayCounters =
    previousByGatewayId[normalizedGatewayId] && typeof previousByGatewayId[normalizedGatewayId] === 'object'
  ...
  const nextGatewayCounters = {
    ...
    [eventName]:
      Math.max(
        0,
        Number(currentGatewayCounters[eventName] ?? 0),
      ) + 1,
  };
