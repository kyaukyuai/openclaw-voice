...
} from '../ui/runtime-logic';
import {
  isMacDesktopRuntime,
  supportsSpeechRecognitionOnCurrentPlatform,
} from '../utils';

type UseHomeUiStateInput = {
  transcript: string;
...
export function useHomeUiState(input: UseHomeUiStateInput) {
  ...
  const visibleSessions = useMemo(() => {
    const active = input.activeSessionKey;
    const merged = [...input.sessions];
    ...
    merged.sort((a, b) => {
      ...
      const aPinned = input.sessionPreferences[a.key]?.pinned === true;
      const bPinned = input.sessionPreferences[b.key]?.pinned === true;
  ...
  const gatewayDiagnosticIconName: QuickTextIcon =
    input.gatewayConnectDiagnostic?.kind === 'tls'
      ...
      : input.gatewayConnectDiagnostic?.kind === 'auth'
        ? 'key-outline'
        : input.gatewayConnectDiagnostic?.kind === 'timeout'
          ? 'time-outline'
          : input.gatewayConnectDiagnostic?.kind === 'dns'
            ? 'globe-outline'
            : input.gatewayConnectDiagnostic?.kind === 'network'
              ? 'cloud-offline-outline'
              : input.gatewayConnectDiagnostic?.kind === 'server'
                ? 'server-outline'
                : input.gatewayConnectDiagnostic?.kind === 'pairing'
                  ? 'people-outline'
                  : input.gatewayConnectDiagnostic?.kind === 'invalid-url'
                    ? 'link-outline'
  ...
  const historyItems = useMemo<HistoryListItem[]>(() => {
    ...
    input.chatTurns.forEach((turn, index) => {
      const dayKey = input.getHistoryDayKey(turn.createdAt);
      if (dayKey !== previousDayKey) {
        items.push({
          kind: 'date',
          id: `date-${dayKey}`,
          label: input.getHistoryDayLabel(turn.createdAt),
        });
      ...
      items.push({
        ...
        id: turn.id,
        turn,
        isLast: index === input.chatTurns.length - 1,
      });
    });
  ...
  const latestRetryText = useMemo(() => {
    const currentDraft = (
      input.transcript.trim() || input.interimTranscript.trim()
    ).trim();
    if (currentDraft) return currentDraft;

    for (let index = input.chatTurns.length - 1; index >= 0; index -= 1) {
      const turn = input.chatTurns[index];
      if (
