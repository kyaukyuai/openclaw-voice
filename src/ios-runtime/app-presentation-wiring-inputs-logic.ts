...
import type { useKeyboardUiRuntime } from './useKeyboardUiRuntime';
import type { useAppViewModelWiring } from './useAppViewModelWiring';
...
type KeyboardUiRuntimeInput = Parameters<typeof useKeyboardUiRuntime>[0];
type AppContentWiringInput = Parameters<typeof useAppContentWiring>[0];
type AppViewModelWiringInput = Parameters<typeof useAppViewModelWiring>[0];

type BuildAppPresentationInput = {
...
type BuildAppViewModelWiringInput = BuildAppPresentationInput & {
  appContent: AppContentState;
  keyboardBarAnim: ReturnType<typeof useKeyboardUiRuntime>['keyboardBarAnim'];
};

export function buildAppContentWiringInput(
  input: BuildAppPresentationInput,
): AppContentWiringInput {
  const {
    settings,
    gateway,
    theme,
...
export function buildKeyboardUiRuntimeInput(input: {
  appContent: AppContentState;
  composerRuntime: ComposerRuntimeState;
  appState: AppRuntimeState;
}): KeyboardUiRuntimeInput {
  return {
...
export function buildAppViewModelWiringInput(
  input: BuildAppViewModelWiringInput,
): AppViewModelWiringInput {
  const {
    settings,
    gateway,
    theme,
  ...
  return {
    ...
    ui: {
      ...
      isSessionHistoryLoading:
        gatewayRuntimeController.state.isSessionHistoryLoading,
      isMissingResponseRecoveryInFlight:
        gatewayRuntimeController.state.isMissingResponseRecoveryInFlight,
      isKeyboardBarMounted: appState.isKeyboardBarMounted,
      showScrollToBottomButton: appState.showScrollToBottomButton,
